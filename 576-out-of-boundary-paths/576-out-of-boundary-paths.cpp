...
public:
    int dp[52][52][52];
    const int mod = 1e9+7;
    int func(int i,int j,int m,int n,int moves,int maxMoves)
    {
        if(i<0 || j<0 || i>=m || j>=n)
        {
            return 1;
        ...
        val = (val+func(i+1,j,m,n,moves+1,maxMoves))%mod;
        val = (val+func(i,j+1,m,n,moves+1,maxMoves))%mod;
        val = (val+func(i-1,j,m,n,moves+1,maxMoves))%mod;
        val = (val+func(i,j-1,m,n,moves+1,maxMoves))%mod;
    ...
    int findPaths(int m, int n, int maxMove, int startRow, int startColumn) {
        ...
        return func(startRow,startColumn,m,n,0,maxMove);
    }
};